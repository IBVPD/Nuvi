function PunkAveFileUploader(options)
{
  ...
  editor.fileupload({
    ...
    drop: function(e, data)
    {
        ...
        {
            editor.lastFile = {name: file.name, uniq: (file.name+file.size).replace(/[^\w\s]/gi, '')};
        });
    },
    ...
    start: function (e) {
        $el.find('.controls').append('<span id="uploader-'+editor.lastFile.uniq+'" class="file-name uploader-file-name"><i class="icon-file"></i>'+editor.lastFile.name+'<img class="" src="/bundles/punkavefileuploader/images/spinner.gif" alt="" /></span>');
      self.uploading = true;
    },
    stop: function (e) {
      self.uploading = false;
    ...
    always: function(e, data) {
        $.each(data.files, function(i, file)
        {
            var str = (file.name+file.size).replace(/[^\w\s]/gi, '');
            $('#uploader-'+str).find('img').hide();
        });
    }
  });
