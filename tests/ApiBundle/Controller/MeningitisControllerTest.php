...
class MeningitisControllerTest extends WebTestCase
{
    const ID = 'CA-ALBCHLD-15-000078';
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPatchCase');
        $client = $this->getClient();
        $client->request('PATCH', $route, [], [], [], '{"case":{"lastName":"Fabien","gender":"1"}}');

        $response = $client->getResponse();
        if($response->getStatusCode() !== 204) {
            file_put_contents('/tmp/patchMeningitis.log', sprintf("%s\n\n%s",self::ID, $response->getContent()));
        }
    ...
    {
        $route  = $this->getUrl('nsApiMeningitisPostCase');
        $client = $this->getClient();
        $client->request('POST', $route, [], [], [], '{"create":{"caseId":"ANewCaseId","type":1,"site":"ALBCHLD"}}');

        $response = $client->getResponse();
        if($response->getStatusCode() !== 204) {
            file_put_contents('/tmp/postMeningitis.log', sprintf("%s\n\n%s",self::ID, $response->getContent()));
        }
    ...
    {
        $route  = $this->getUrl('nsApiMeningitisPatchLab', ['objId' => self::ID]);
        $client = $this->getClient();
        $client->request('PATCH', $route, [], [], [], '{"site_lab":{"bloodId":"ANewCaseId","bloodCultDone":0}}');

        $response = $client->getResponse();
        if (!$response->isSuccessful()) {
            file_put_contents('/tmp/Meningitis-lab.html', $response);
        }
    ...
    public function testGetRRLCase()
    {
        ...
        if ($response->getStatusCode() == 500) {
            file_put_contents('/tmp/nsApiMeningitisGetRRL.log', $response->getContent());
        }
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPatchRRL');
        $client = $this->getClient();
...
//        $this->assertEquals("ANewCaseId", $decoded['LabId']);
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPutRRL');
        $client = $this->getClient();
...
//        $this->assertArrayHasKey('SampleType', $decoded);
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisGetNL');
        $client = $this->getClient();
        $client->request('GET', $route);
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPatchNL');
        $client = $this->getClient();
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPutNL');
        $client = $this->getClient();
    ...
    {
        $route  = $this->getRoute('nsApiMeningitisPutCase');
        $client = $this->getClient();
